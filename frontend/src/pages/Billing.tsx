  ...
  const handleBillGeneration = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        // Try to download/print the invoice PDF
        try {
          const blob = await apiService.getSalePDF(response.data._id);
          const url = URL.createObjectURL(blob);

          // Open in new window with embed for print-friendly display
          const w = window.open("about:blank");
          if (w) {
            w.document.write(
              `<html><head><title>Invoice ${response.data.invoiceNumber}</title></head><body style="margin:0"><embed width='100%' height='100%' src='${url}' type='application/pdf'></embed></body></html>`
            );
            ...
            // Revoke URL after some time to allow embed to load
            ...
            a.href = url;
            a.download = `invoice-${
              response.data.invoiceNumber || response.data._id
            }.pdf`;
          ...
          console.error("Failed to download invoice PDF:", pdfError);
          toast({
            title: "Notice",
        ...
        toast({
          title: "Error",
          description: response.message || "Failed to generate bill",
          type: "error",
        });
      }
    } catch (error: any) {
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Create bills and process sales transactions
              </p>
            </div>
