...
export function SalesChart({ className = '' }: SalesChartProps) {
  ...
  const fetchChartData = async () => {
    ...
    try {
      const [, dashboardStats, salesData] = await Promise.all([
        apiService.getSalesStats(selectedPeriod === '7d' ? 7 : selectedPeriod === '30d' ? 30 : 365),
      ...
      if (dashboardStats.success && dashboardStats.data) {
        ...
        const categoryChartData = categoryStats.slice(0, 8).map((cat: any, index: number) => ({
  ...
  const fetchSalesDataForPeriod = async (period: string): Promise<SalesData[]> => {
  ...
  const aggregateSalesData = (sales: any[], period: string, days: number): SalesData[] => {
    const isYearly = period === '1y';
    const salesMap = new Map<string, { sales: number; revenue: number; profit: number }>();
    ...
    <div className={`bg-white rounded-xl shadow-sm p-6 ${className}`}>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">Track your business performance over time</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6 pt-6 border-t border-gray-100">
        <div className="text-center">
          ...
          <div className="font-semibold text-gray-900">
            {formatTooltipCurrency(salesData.reduce((sum, item) => sum + item.revenue, 0))}
          </div>
        </div>
        <div className="text-center">
          <div className="flex items-center justify-center gap-1 mb-1">
          ...
          <div className="font-semibold text-gray-900">
            {formatTooltipCurrency(salesData.reduce((sum, item) => sum + item.profit, 0))}
          </div>
        </div>
        <div className="text-center">
          <div className="flex items-center justify-center gap-1 mb-1">
          ...
          <div className="font-semibold text-gray-900">
            {selectedPeriod === '7d' ? '7 Days' : selectedPeriod === '30d' ? '30 Days' : '1 Year'}
          </div>
        </div>
        <div className="text-center">
          <div className="flex items-center justify-center gap-1 mb-1">
